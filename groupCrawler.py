...
while flag:
	...
	cursor = db.cursor()
	cursor.execute("select user_name from lastfm_crawlqueue where (crawl_flag=1 or crawl_flag=3 or crawl_flag=4 or crawl_flag=5) and groups=0 limit 1;")
	...
	if result:
		...
		cursor=db.cursor()
		cursor.execute("update lastfm_crawlqueue set groups=1 where user_name=%s",(username))
		closeDBConnection(cursor)
		print username, uid, datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
		log = open('groupLog','a')
		log.write(','.join([str(i) for i in [username,uid,datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")]])+'\n')
		log.close()
		htmlParsingMethods.getUserGroups(username,uid)	
	else:
